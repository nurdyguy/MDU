...
StatInterface.prototype.DisplayResults = function (result)
{
    ...
    {
        str = 'Won: ' + numberWithCommas(result[i].wins) 
            + '<br>Lost: ' + numberWithCommas(result[i].loses) 
            + '<br>Chopped: ' + numberWithCommas(result[i].chops);
        $('#handPlayer' + (i + 1)).find('a.popoverData').attr('data-content', str).html('&nbsp;' + (result[i].wins * 100 / (result[i].wins + result[i].loses + result[i].chops)).toFixed(1) + '% ').show();
    }
}
